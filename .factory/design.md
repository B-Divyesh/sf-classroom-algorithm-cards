...
Asset generation uses the factory image model deployment `factory-image` through `/opt/fleet/lib/gen-image.sh`, generated 2026-08-27. Generated imagery is original for this product. The exact prompt is retained beside the source image in `assets/src/hero-cards.json`; optimized WebP and AVIF derivatives ship with the site. The AVIF is a 960 × 640 derivative of that source, encoded on 2026-09-05 with `avifenc` (libavif 1.0.4) at 18,063 bytes on disk.
